<?php
    ...
    $borrowId = isset($_POST['borrow_id']) ? trim($_POST['borrow_id']) : '';
    ...
    $borrowStatus = isset($_POST['borrow_status']) ? trim($_POST['borrow_status']) : '';
    $borrowDate = isset($_POST['borrow_date']) ? trim($_POST['borrow_date']) : '';
    ...
    if ((isset($_POST['book_action']) && $_POST['book_action'] === 'edit') || isset($_POST['edit_borrow'])) {
        ...
        if ($stmt = $conn->prepare('UPDATE bookborrower SET book_id = ?, member_id = ?, borrow_status = ?, borrower_date_modified = ? WHERE borrow_id = ?')) {
            $stmt->bind_param('sssss', $bookId, $memberId, $borrowStatus, $borrowDate, $originalBorrowId);
            if ($stmt->execute()) {
        ...
        if ($stmt = $conn->prepare('INSERT INTO bookborrower (borrow_id, book_id, member_id, borrow_status, borrower_date_modified) VALUES (?, ?, ?, ?, ?)')) {
            $stmt->bind_param('sssss', $borrowId, $bookId, $memberId, $borrowStatus, $borrowDate);
            if ($stmt->execute()) {
